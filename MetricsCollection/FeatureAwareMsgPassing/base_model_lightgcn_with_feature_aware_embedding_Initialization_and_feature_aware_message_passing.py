...
# ----------------------------------------------------------------------
# Feature-informed embedding initialization (keep this!)
# ----------------------------------------------------------------------
...
# Create feature matrix for all nodes (playlists get zero features)
...
print(f"Feature-aware message passing: Enabled")
...
# ----------------------------------------------------------------------
# Feature-Aware Message Passing Layer
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Enhanced GNN Model with Feature-Aware Message Passing
# ----------------------------------------------------------------------
class FeatureAwareGNN(torch.nn.Module):
    ...
    def calc_loss(self, mp, pos, neg, node_features):
        embs = self.gnn_propagation(mp, node_features)
    ...
    def evaluation(self, mp, pos, k, node_features):
        embs = self.gnn_propagation(mp, node_features)
        pl_emb, song_emb = embs[:self.num_playlists], embs[self.num_playlists:]
        ratings = torch.sigmoid(pl_emb @ song_emb.T)
        return gpu_recall(mp.edge_index, ratings, pos.edge_index.to(device), self.num_playlists, k)
...
# ----------------------------------------------------------------------
# Train / eval with feature-aware model
# ----------------------------------------------------------------------
def train(model, mp, loader, opt, node_features):
    model.train()
    tot_loss = 0
    tot_edges = 0
    ...
    for batch in loader:
        ...
        if use_amp and scaler is not None:
            with autocast(**AMP_KW):
                loss = model.calc_loss(mp, batch, neg, node_features)
...
@torch.no_grad()
def evaluate(model, mp, loader, k, node_features):
    model.eval()
    recs = []
    ...
    with autocast(**AMP_KW) if use_amp else torch.cuda.amp.autocast(enabled=False):
        for batch in loader:
            batch = batch.to(device)
            recs.append(model.evaluation(mp, batch, k, node_features))
...
# ----------------------------------------------------------------------
# Main loop with feature-aware model
# ----------------------------------------------------------------------
model = FeatureAwareGNN(emb_dim=64, num_nodes=data.num_nodes,
                        num_playlists=num_playlists, layers=3,
                        song_features=song_features, feature_dim=feature_dim).to(device)
tracker = MetricsTracker("feature_aware")
tracker.log_hyperparameters(
    ...
    feature_aware_mp="Enabled",
    feature_informed_init="Enabled",
    num_features=feature_dim,
    use_amp=use_amp,
    amp_dtype=str(amp_dtype)
)
tracker.log_model_stats(model)
...
history = []
print("Starting training with Feature-Aware Message Passing...")
for epoch in range(epochs):
    tracker.start_epoch()  # ADD THIS
    
    tr_loss, train_edges, train_time = train(model, train_mp, train_loader, opt, feature_matrix)
    ...
    if epoch in range(11) or epoch % 5 == 0:
        val_rec, val_time, val_edges = evaluate(model, val_mp, val_loader, k, feature_matrix)
        
        tracker.end_epoch(epoch, tr_loss, val_rec, train_edges, train_time)  # ADD THIS - Note: passing train_time
        ...
        if val_rec > best_val:
            ...
            print(f"  ↳ New BEST val_recall={val_rec:.4f} at epoch {epoch}. Saved {best_ckpt}")
            if stop_on_first_best:
                print("  ↳ Stopping on first best as requested.")
                break
        # Also run test evaluation during training (keeping original behavior)
        #########################################################################################################
        #test_rec, test_time, test_edges = evaluate(model, test_mp, test_loader_fn(), k, feature_matrix)
        #print(f"\nTest recall@{k}={test_rec:.4f}, test_time={test_time:.2f}s")
    else:
        tracker.end_epoch(epoch, tr_loss, train_edges=train_edges, train_time=train_time)  # ADD THIS - Note: passing train_time
...
# Final test
# ----------------------------------------------------------------------
# Ensure we test with the saved best validation weights
...
test_rec, test_time, test_edges = evaluate(model, test_mp, test_loader, k, feature_matrix)
...
tracker.log_test_results(test_rec, test_time)
tracker.save()
...
plt.title("Loss over time (Feature-Aware Message Passing)")
plt.grid(True)
plt.show()
...
}, 'feature_aware_lightgcn_checkpoint.pt')
print("\nModel saved to feature_aware_lightgcn_checkpoint.pt")
